...
export class AppComponent {
  ...
  dummyArray = Array(100).fill(this.dummy);
  dummDate = {
  ...
  private toOTTemplate() {
    ...
    this.dummyArray.forEach((element, idx) => {
      let dateObj = {
        no: idx + 1,
        ...element,
      ...
      dateObj[moment(this.dummDate.sdate).format('DD/MM')] =
        this.dummDate.content;
      rows.push(dateObj);
    });
    //
    const sumRow = this.sumObjDate(startDate, endDate, rows);
    ...
    // export file
    this.autofitColumns(rows, worksheet, header);
    ...
    for (let i = 0; i < json.length; i++) {
      ...
      for (let j = 0; j < jsonKeys.length; j++) {
        const l = value[jsonKeys[j]]
          ? value[jsonKeys[j]]?.toString().length
          : 0;
  ...
  createObjDate(startDate: moment.Moment, endDate: moment.Moment) {
  ...
  // suym by date
  sumObjDate(startDate: moment.Moment, endDate: moment.Moment, rows: any[]) {
    let obj: any[] = [];
    let tickDate = moment(startDate).clone();
    while (endDate.diff(tickDate, 'days') >= 0) {
